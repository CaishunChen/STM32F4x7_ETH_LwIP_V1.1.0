 ...
 * 修改日期:2015年4月29日    
 *******************************************************************************/ 
void SaveMenuDetail(void)
{
   __IO uint32_t FlashID = 0;
   __IO uint32_t DeviceID = 0;
	 uint8_t TempBuffer[FloorMealNum*6]={0};
  /* Initialize the SPI FLASH driver */
    //SPI_FLASH_Init();
	  //SPI_FLASH_Init();
	  //SPI_FLASH_BufferRead(TempBuffer, SPI_FLASH_Sector0, FloorMealNum*6);
    ...
    /* Get SPI Flash ID */
     FlashID = SPI_FLASH_ReadID(); 
    /* Write Tx_Buffer data to SPI FLASH memory */
	  SPI_FLASH_BufferRead(TempBuffer, SPI_FLASH_Sector1, MealKindTotoal*47);/*参看原始数据*/
	
	  SPI_FLASH_SectorErase(FLASH_SectorToErase);
    SPI_FLASH_BufferWrite(FloorMealMessageWriteToFlash.FlashBuffer, SPI_FLASH_Sector1 , FloorMealNum*6);
    /* Read data from SPI FLASH memory */
	  //delay_ms(10); //延时测试
    SPI_FLASH_BufferRead(TempBuffer, SPI_FLASH_Sector1, MealKindTotoal*47);

    /* Check the corectness of written dada */
    TransferStatus1 = Buffercmp(FloorMealMessageWriteToFlash.FlashBuffer, TempBuffer, MealKindTotoal*47);
		if(TransferStatus1 == FAILED )/*加入可以排错*/
    {
	    SPI_FLASH_BufferWrite(Meal_Union.MealBuf, SPI_FLASH_Sector1, MealKindTotoal*47);
      /* Read data from SPI FLASH memory */
      SPI_FLASH_BufferRead(Meal_Union.MealBuf, SPI_FLASH_Sector1, MealKindTotoal*47);
      /* Check the corectness of written dada */
      TransferStatus1 = Buffercmp(Meal_Union.MealBuf, TempBuffer, MealKindTotoal*47);
    }  
}
